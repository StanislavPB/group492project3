...
public class TestResultService {
    ...
    public Response<Double, String> getBestTestingResultsForStudentCourse(Integer idCourse, Integer idStudent) {

        List<TestResult> testResultList = testResultRepository.getTestingResultsForStudentCourse(idCourse, idStudent);

        if (testResultList.isEmpty()) {
            return new Response<>(null, false, "Tests not passed.");
        } else {
            Double bestPercent = 0.0;
            for (int i = 0; i < testResultList.size(); i++) {

                Double tempPercent = testResultList.get(i).getPercentOfCorrectAnswers();
                if (bestPercent < tempPercent){
